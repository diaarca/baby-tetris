...
public class State {
    ...
    public Tromino getNextTromino() { return this.nextTromino; }
    ...
    public ArrayList<Action> getAvailableActions() {
        ...
        for (Point p : this.field.getEmptyPositions()) {
            for (int r = 0; r < this.nextTromino.rotationCount(); r++) {
                if (this.field.isAvailable(this.nextTromino, p.getX(), p.getY(),
                                           r)) {
                    int[][] offsets = this.nextTromino.getOffsets(r);
                    for (int i = 0; i < offsets.length; i++) {
                        if (placementPosition.contains(
                                new Point(p.getX() + offsets[i][0],
                                          p.getY() + offsets[i][1]))) {
    ...
    public State applyAction(Action action) {
        ...
        // add the new tromino
        newField.addTromino(this.nextTromino, action.getPosition().getX(),
                            action.getPosition().getY(), action.getRotation());

        Tromino newNextTromino =

...
\subsection{Implementation}
\subsubsection{Max - Min Value Iteration (\code{robustActionValueIteration})}
\quad In this variant, the adversary selects the next tromino ('I' or 'L') that results in the lowest average expected reward for the player, averaged over all available actions.
...
for (int k = 0; k < nbActions; k++) {
    ...
    auto nextStates = currState.genAllStatesFromAction(actions[k]);
    if (nextStates.empty()) {
        continue;
    }

    for (State& placedState : nextStates) {
        State afterState = placedState.completeLines();

        auto it = V.find(afterState);
        if (it == V.end()) {
            std::cerr << "ERROR (robustActionValueIteration): the state cannot be derived into a non-reachable state" << std::endl;
            exit(1);
        }
        double vAfter = it->second;
        double reward = placedState.evaluate(config_) + lambda * vAfter;
...
vPrime = *std::max_element(robust_action_values.begin(), robust_action_values.end());
\end{lstlisting}

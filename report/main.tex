...
We have the same states (reachable states), actions, transition function and reward function as in Section~\ref{sec:question1},
the only change is the value iteration equation.
% \[
...
\quad We considered two different heuristic approaches for choosing the tromino piece:
\begin{itemize}
    ...
    expected reward for the player. CHECK THE EQUATION!!!!
    \[
...
\subsection{Implementation}
\quad We add a new method in the \code{MDP} class that computes value iteration for the adversary policy.
LISTING!!!!
\end{document}
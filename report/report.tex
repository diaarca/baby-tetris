...
\usepackage[top=0.6in, bottom=0.6in, left=0.8in, right=0.8in,
headheight=15pt, footskip=10pt]{geometry} % Ensure enough space for
% headers/footers
...
\item \textbf{Transition Function}\\
  ...
  of uniform distribution between the pieces, each transition has an
  equal probability of $1/2$.
\item \textbf{Reward Function and Discount Factor}\\
  The reward function is computed based on the number of lines
  cleared after placing the piece,
  and a discount factor $\lambda$ is used to weigh future rewards.\\
  $R(s, a, s') := \text{coefficient * number~of~lines~cleared}$ with
  $\lambda \in [0, 1]$
\end{itemize}
}

\subsection{Finding Optimal Policy}
\quad We use the value iteration method to find the optimal policy
that maximizes
the expected rewards, iteratively updating the value function for each state
based on the Bellman equation until it converges to the optimal value
(margin $\epsilon$).
\[
V(s) = \max_{a \in A} \sum_{s' \in S} P(s' | s, a) \cdot \left( R(s,
a, s') + \lambda V(s') \right)
\]
Value iteration is better suited for maximizing the expected discounted reward
since it computes the optimal value then extracts the policy and it's
easier to implement.
...
Now that we presented the overview of the global implementation
architecture, we will describe
...
Section~\ref{sec:question1}, the reward function takes both the
...
\quad The value iteration algorithm is implemented for the player by
maintaining a value function
for each state and updating it based on the expected rewards from
taking actions in that state.
The output is a policy that maps each state to the action that
...
keeping in mind that the algorithm is bounded in iteration by the
$maxIteration$.
And the variant variables will be: the height $\in \{4, 5\}$ and the
discount factor
$\lambda \in [0, 1]$.\\
\\
...
gain that much per action, prioritizing safer actions that allow a
higher score on
the long term.
...
shows that bigger grid provides a larger space to take risks in order
to increase
the global score. The game with $height = 5$ and $\lambda = 0.9$
illustrates this
perfectly because it scores higher than the game with $height = 5$ and
$\lambda = 0.1$, since the risky actions can be better expolited on a
bigger grid.
...
\quad Instead of computing the value iteration on all possible
states, we focus on
...
\If{$Q.\text{doesNotContain}(s_{\text{After}})$ \textbf{and}
$H.\text{doesNotContain}(s_{\text{After}})$}
...
\quad We can then remark that with this new implementation, the value
iteration algorithm is quite slower both in terms of computation (per
iteration) and in number of iteration. For instance, the 4$\times$4
game with $\lambda = 0.4$ wasn't able to play all the 10000 actions
with being blocked while we can now play it with a way more "risky"
policy ($\lambda = 0.9$).

\section{Question 2: Adversarial MDP For Opponent}
\quad The goal of this part is to design an adversary policy that
selects tromino pieces for the game
with the intention of minimizing the players score.
For this policy we use once again value iteration, but this time we
minimize the expected cumulative
discounted reward over time from the available actions. The policy
chooses the tromino piece that
leads to the lowest expected score for the player.
We have the same states (reachable states) and reward function as in
Section~\ref{sec:question1}.
{\renewcommand\labelitemi{}
\begin {itemize}
\item \textbf{Actions}\\
The actions for the adversary correspond to the choice of the next
tromino piece for the player to place on the grid.\\
A := \{I, L\}
\item \textbf{Transition Function}\\
The transition function for the adversary is defined as :\\
\[P(s' | s, a, t) := 1/|A_t(s)|\]
where $A_t(s)$ is the set of available actions for the player given
the current state $s$ and the chosen tromino piece $t$,
with the assumption of a uniform distribution over the player's
possible actions for the given piece.
\end {itemize}
}
% \[
% V(s) = \min_{a \in A} \sum_{s' \in S} P(s' | s, a) \cdot \left(
% R(s, a, s') + \lambda V(s') \right)
% \]
The player will still use the optimal policy computed in
Section~\ref{sec:question1} to play the game.

\subsubsection{Choosing the Tromino Piece}
\quad We considered two different heuristic approaches for choosing
the tromino piece, their value iterations are defined as:
\begin{itemize}
\item \textbf{Lowest Maximal Expected Reward}: The policy chooses the
tromino piece that leads to the lowest maximal
expected reward for the player.
\[
  V(s) = \min_{t \in \{I,L\}} \max_{a \in A_t(s)} \sum_{s'}
  P(s'|s,a,t) \cdot \left( R(s,a,s') + \lambda V(s') \right)
\]

\item \textbf{Lowest Average Expected Reward}: The policy chooses the
tromino piece that leads to the lowest average
expected reward for the player.
\[
  V(s) = \min_{t \in \{I,L\}} \frac{1}{|A_t(s)|} \sum_{a \in A_t(s)}
  \sum_{s'} P(s'|s,a,t) \cdot \left( R(s,a,s') + \lambda V(s') \right)
\]
\end{itemize}
Both approaches produce similar results, with no remarkable
difference. However, compared to the random
adversary policy, where the tromino pieces are chosen randomly, the
tromino value iteration, for both approaches,
successfully decreases the player's score.  \\

% \subsection{Implementation}
% \quad We add a new method in the \code{MDP} class that computes
% value iteration for the adversary policy. Both the
% \code{trominoValueIterationMinMax} and
% \code{trominoValueIterationMinAvg} methods share a common value
% iteration framework. They primarily differ in how the next tromino
% piece is chosen by the adversary, which in turn defines the update
% rule for the state value function $V(s)$. The common part involves
% an outer loop for value iteration convergence and an inner loop
% iterating through all reachable states via a map `V`.

% \subsubsection{Lowest Maximal Expected Reward
% (\code{trominoValueIterationMinMax})}
% \quad In this approach, the adversary aims to minimize the maximum
% possible expected reward that the player can achieve. For each
% state, the algorithm computes the best outcome for the player for
% each potential next piece. The adversary then chooses the piece
% that leads to the lower of these two maximal rewards.
...
% \subsubsection{Lowest Average Expected Reward
% (\code{trominoValueIterationMinAvg})}
% \quad In this variant, the adversary selects the next tromino ('I'
% or 'L') that results in the lowest average expected reward for the
% player, averaged over all available actions.
...
\quad According to our implementation, we made the same executions in
order to compare our adversary policies. Here we play the value
iteration policy (from Section~/ref{sec:question1}) for the player on
3 adversary policies: random (based on \code{PROBA\_I\_PIECE}),
Min-Max value iteration and Min-Avg iteration:
...
\subsubsection{Observations}
\quad We can first observe that the score is not that much reduced by
the adversary policies even if it is more noticeable in the
4$\times$5 case. This is caused by the lack of possibilities, we can
only choose between 2 pieces where the player may have to choose
between way more actions.

Contrarily to the player value iteration we can remark that the
$\lambda$ parameter doesn't affect the score of the game. This is due
to the fact that the score is minimized in that case and then the
factor with which we multiply the $V(s)$ will not change anything.

\section{Question 3: Robust Policy For Player}
\quad In this section, we study the case where the player doesn't make any
assumptions on the adversary policy, searching for a generally
suitable policy instead
of optimizing against a specific one. We accept a lower maximum score
if it elevates
the minimal score across all adversary policies.\\

\quad To this end, the first idea we had was to implement a Max-Min
value iteration.
Player policy always considers the worst-case scenario against all
adversary policies.
This is achieved by modifying the value iteration to maximizing the
minimum expected reward
over all possible states resulting from the adversary's actions
(tromino choices).
\[
V(s) = \max_{a \in A} \min_{t \in \{I,L\}} P(s' | s, a, t) \cdot
\left( R(s, a, s') + \lambda V(s') \right)
\]

\quad The second approach we had was to parameterize the base player
value iteration.
The idea is to use these parameters as weight in the reward function
to adjust the importance of each factor.
\[R(s, a, s') := \text{lw} \cdot \text{completed\_lines}_{s'} -
\text{hw} \cdot \text{max\_cell\_height}_{s'} +
\text{sw} \cdot \text{score}_{s'} -
\text{gr} \cdot \text{gaps}_{s'}
\]
Where \code{lw}, \code{hw}, \code{sw} and \code{gr} are respectively
the weights for completed lines,
the highest occupied cell's height, the produced score and the number
of gaps obtained after the tromino placement.

\subsection{Executions}
\quad The following table presents selected results from our grid
search experiments for the parameterized policy and the performance
of the dedicated Max-Min Value Iteration (VI) policy.
...
\textbf{Rand} & \textbf{MinMax} & \textbf{MinAvg} & \textbf{GapAvg} &
\textbf{Min} \\
\hline
\multicolumn{4}{|c|}{\textbf{Max-Min VI (Fixed Parameters)}} & 10685
& 9999 & 9999 & 10356 & 9999 \\
\hline
...
20 & 0 & 4 & 1 & 9242.76 & 7499 & 10000 & 7499 & 7499 \\
0 & 1 & 4 & 0 & 10046.42 & 9999 & 10000 & 8749 & 8749 \\
0 & 1 & 4 & 1.5 & 9810.65 & 9999 & 10000 & 9999 & 9810.65 \\
10 & 2 & 1 & 0 & 7856.08 & 7499 & 8750 & 7499 & 7499 \\
20 & 2 & 3 & 1 & 7934.26 & 9996 & 8750 & 7499 & 7499 \\
0 & 2 & 1 & 1 & 0.05 & 0 & 0 & 0 & 0 \\
\hline
\end{tabular}
\caption{Selected Results from Parameter Grid Search and Max-Min VI}
\label{tab:grid_search_results}
\end{table}

\subsection{Observations}
\quad The grid search across the four parameters, alongside the
dedicated Max-Min Value Iteration, revealed several key insights into
what constitutes a robust and high-performing policy.

\begin{itemize}
\item \textbf{Height Weight is Critical}: The most dramatic results
are seen with the \code{height\_weight}. Configurations where this
parameter is greater than zero perform exceptionally poorly, often
failing to achieve any significant score. This indicates that a
strong penalty for increasing the board's height is
counterproductive, as the agent becomes too conservative to clear
lines effectively. The best-performing policies consistently had a
\code{height\_weight} of 0.

\item \textbf{Score and Line Weights Matter}: High scores were
achieved when the \code{score\_weight} was a dominant factor,
especially when both \code{line\_weight} and \code{height\_weight}
were zero. For instance, configuration \{0, 0, 3, 0\} showed strong
performance, suggesting that directly optimizing for the game's
intrinsic score (which is based on line clears) is a very effective
strategy. A high \code{line\_weight} also performs well but can be
less consistent against different adversaries if not balanced.

\item \textbf{Max-Min VI Performance}: The
\code{robustActionValueIterationMaxMin} achieved a commendable
minimal score of 9999, matching some of the best grid-searched
configurations. Its performance against the Random adversary was
10685, which is solid, though some parameter-tuned policies (e.g.,
\{0, 0, 3, 0\}) slightly surpassed it. Against MinMax and MinAvg
adversaries, it consistently achieved 9999, indicating good inherent
robustness. This confirms the effectiveness of the Max-Min approach
as a baseline for robust play.

\item \textbf{Robustness vs. High Score}: Comparing the "Most Robust"
policy from the parameter search (configuration \#0 \{0, 0, 1, 0\}
with a min score of 9999) with the explicit Max-Min VI, both achieve
the same minimal score against specific adversaries. However, the
Max-Min VI exhibits a slightly higher average performance against the
random adversary, making it a strong contender for overall robustness
without requiring extensive parameter tuning.

\item \textbf{Variability}: Configurations that were highly variable
(e.g., configuration \{10, 0, 3, 1\}) demonstrated policies
specialized for certain adversaries (e.g., MinAvg) but with lower
scores against others. This highlights the trade-off between a
specialized, high-reward policy and a generally robust one.

\item \textbf{Gap Reduction}: The \code{gap\_reduction} parameter had
a more subtle effect. While not as impactful as the other weights, a
small penalty for gaps (e.g., a weight of 1.0) often helped stabilize
the policy. However, a weight of 0 for this parameter was part of
some of the best overall configurations, suggesting its importance is
secondary to a strong line-clearing and scoring incentive, or its
optimal value is very low.
\end{itemize}

\quad In conclusion, the experiments show that a robust policy for
this game should primarily focus on maximizing the score derived from
line completions while completely avoiding penalties for board
height. The explicit Max-Min Value Iteration provides a strong robust
baseline. Parameter tuning allows for fine-grained control and can
yield slightly higher average scores against random adversaries, but
the core principle of robustness against the worst-case remains paramount.
% \section{Usage}
% \quad Different constants can be adjusted by modifying the
% corresponding \code{\#define}
